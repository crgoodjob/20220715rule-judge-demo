...
public final class AttachmentExportUtil {
    ...
    private static void setAttachmentConfig(String fileName, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        response.addHeader("Content-Disposition", "attachment;filename*=UTF-8''" + URLEncoder.encode(fileName, CharEncoding.UTF_8).replace("+", "%20"));
    }
     ...
     * @param response HttpServletResponse
     */
    public static void exportWorkbook(Workbook workbook, String fileName, HttpServletResponse response) {
        try {
            String suffix = Constants.XLSX;
            if (workbook instanceof HSSFWorkbook) {
                if (fileName.endsWith(suffix)) {
